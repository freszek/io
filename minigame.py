...
class Tap(pygame.sprite.Sprite):
    def __init__(self, x, y, water_flow_rate=0.1):
        super().__init__()
        self.image = pygame.Surface((TAP_SIZE, TAP_SIZE))
        self.rect = self.image.get_rect()
    ...
    def update(self):
        if not self.zakrecony:
            ...
            self.image = pygame.transform.rotate(pygame.Surface((TAP_SIZE, TAP_SIZE)), self.angle)
        self.update_color()
...
def show_start_screen(instruction_text, screen):
    screen.fill((0, 0, 0))
...
def run_game():
    global WIDTH, HEIGHT, WATER_COUNTER
    ...
    instruction_text = [
        ...
        "Pamiętaj, że oszczędzanie wody to ważny krok dla naszej planety!",
        "Kliknij dowolny klawisz lub lewy przycisk myszy, aby rozpocząć.",
        "Powodzenia!"
    ]

    show_start_screen(instruction_text, screen)
