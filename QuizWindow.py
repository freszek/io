...
class QuizWindow:
    def __init__(self):
        ...
        self.event = event
        self.root = tk.Tk()
        self.root.title(event.get_name() + " Quiz")
        self.root.geometry("600x300")
        self.root.configure(background="#90EE90")
    ...
    def choose_level(self):
        self.destroy_window()
        level_label = ttk.Label(self.root, text="Wybierz poziom")
        level_label.pack()

        self.level_var = tk.StringVar()
        levels = ["EASY", "MEDIUM", "HARD"]
        for level in levels:
            ttk.Radiobutton(self.root, text=level, variable=self.level_var, value=level).pack()
        level_button = ttk.Button(self.root, text="Rozpocznij quiz", command=self.start_quiz)
        level_button.pack()
    ...
    def show_rules(self):
        ...
        rules_label = ttk.Label(self.root, text=s)
        rules_label.pack()

        rules_button = ttk.Button(self.root, text="Zacznijmy!", command=self.choose_level)
        rules_button.pack()
    ...
    def start_quiz(self):
        ...
        self.question_label = ttk.Label(self.root, text=self.questions[0][2])
        self.question_label.pack()

        self.var = tk.IntVar()
        answers = self.questions[0][3:7]
        for i, answer in enumerate(answers):
            ttk.Radiobutton(self.root, text=answer, variable=self.var, value=i).pack()

        self.next_button = ttk.Button(self.root, text="Dalej", command=self.next_question)
    ...
    def update_question(self):
        self.destroy_window()
        self.question_label = ttk.Label(self.root, text=self.questions[self.current_question][2])
        self.question_label.pack()
        self.var.set(0)
